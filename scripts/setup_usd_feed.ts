...
async function main() {
  const { ORACLE, FEED, DAI, USDC, USDT, DAI_V2, MAI, BUSD } = await getNamedAccounts()
  let oracle, tx
  ...
  if (network.name === 'andromeda') {
    ...
    tx = await oracle.setAssetSources([DAI, USDC, USDT, DAI_V2, MAI, BUSD], [FEED, FEED, FEED, FEED, FEED, FEED])
    await tx.wait()
  }
}
